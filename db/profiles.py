...
class ProfileStore:
    ...
    def get_profile(self, student_id: str) -> Optional[IdentityProfile]:
    ...
    def save_profile(self, profile: IdentityProfile, reason: str = "update") -> None:
        ...
        self.connection.execute(
            """INSERT INTO profile_versions (student_id, version, data, reason, created_at)
               VALUES (?, ?, ?, ?, ?)""",
            (profile.student_id, profile.profile_version, data_json, reason, time.time())
        )
        self.connection.commit()
